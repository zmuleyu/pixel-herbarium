...
interface SharePosterPlant {
  ...
  name_latin: string;
  rarity: number;
  hanakotoba: string;         // empty string = hide hanakotoba section
  bloom_months: number[];     // needed for getPlantGradientColors()
...
6. Floral divider (── ✿ ──)
7. "花言葉" label + hanakotoba in 「brackets」 (hidden entirely when hanakotoba is empty)
...
+   discoveryCity={discoveries[0]?.city}   // requires adding city to DiscoveryRecord + select query
+ />
```
...
+   onClose={() => setShareSheetVisible(false)}
+   plant={{
+     name_ja: discovery.discoveredPlant.name_ja,
+     name_latin: discovery.discoveredPlant.name_latin,
+     rarity: discovery.discoveredPlant.rarity,
+     hanakotoba: discovery.discoveredPlant.hanakotoba ?? '',
+     bloom_months: discovery.discoveredPlant.bloom_months ?? [],
+     pixel_sprite_url: discovery.discoveredPlant.pixel_sprite_url,
+     cityRank: discovery.discoveredPlant.cityRank ?? null,
+   }}
+   discoveryDate={new Date().toISOString()}
+   discoveryCity={discovery.discoveredPlant.city}   // city from reverse geocode during save
+ />
```
