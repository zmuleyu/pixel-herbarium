...
// Mutable mock for useSeasonRecap — toggled per-test
const mockRecap = jest.fn(() => ({
  plants: [{ id: 1, name_ja: '\u685c', rarity: 1, pixel_sprite_url: null, hanakotoba: null }],
  loading: false,
  season: { emoji: '\ud83c\udf38', label: '\u685c\u306e\u5b63\u7bc0', season: 'spring' },
}));

// Mock all dependencies BEFORE imports
jest.mock('react-i18next', () => ({
  useTranslation: () => ({ t: (key: string, params?: any) => params?.count != null ? `${key}:${params.count}` : key }),
}));
...
function renderToString(): string {
  const element = React.createElement(RecapScreen);
...
describe('RecapScreen', () => {
  ...
  it('renders season section with emoji', () => {
    const output = renderToString();
    expect(output).toContain('\ud83c\udf38');
  });

  it('renders plant list when plants exist', () => {
    const output = renderToString();
    expect(output).toContain('\u685c');
  });

  it('shows share button', () => {
    const output = renderToString();
    expect(output).toContain('common.share');
  });

  it('handles empty plants (no items message)', () => {
    mockRecap.mockReturnValue({
      plants: [],
      loading: false,
      season: { emoji: '\ud83c\udf38', label: '\u685c\u306e\u5b63\u7bc0', season: 'spring' },
    ...
    const output = renderToString();
    expect(output).toContain('herbarium.recapEmpty');
  });
