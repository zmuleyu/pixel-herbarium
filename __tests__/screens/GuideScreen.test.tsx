...
jest.mock('@/utils/guide-storage', () => ({
  resetGuide: jest.fn().mockResolvedValue(undefined),
...
function renderToString(): string {
  const element = React.createElement(GuideScreen);
...
describe('GuideScreen', () => {
  ...
  it('shows feature cards with i18n keys', () => {
    const output = renderToString();
    expect(output).toContain('guide.settings.featureDiscover');
    expect(output).toContain('guide.settings.featureStamp');
    expect(output).toContain('guide.settings.featureHerbarium');
    expect(output).toContain('guide.settings.featureMap');
  });
  ...
  it('shows reset all guides button', () => {
    const output = renderToString();
    expect(output).toContain('guide.settings.resetAll');
  });

  it('renders navigation header', () => {
    const output = renderToString();
    expect(output).toContain('guide.settings.title');
  });
});
