...
import { WindowStamp } from '@/components/stamps/WindowStamp';
...
function renderToJson(props: Parameters<typeof WindowStamp>[0]): string {
  const element = React.createElement(WindowStamp, props);
...
describe('WindowStamp', () => {
  ...
  it('renders custom text when provided', () => {
    const output = renderToJson({ ...props, customText: 'Silver pavilion' });
    expect(output).toContain('Silver pavilion');
  });

  it('omits custom text when absent', () => {
    const output = renderToJson(props);
    expect(output).not.toContain('Silver pavilion');
  });
});
