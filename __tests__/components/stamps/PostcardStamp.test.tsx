...
import { PostcardStamp } from '@/components/stamps/PostcardStamp';
...
function renderToJson(props: Parameters<typeof PostcardStamp>[0]): string {
  const element = React.createElement(PostcardStamp, props);
...
describe('PostcardStamp', () => {
  ...
  it('renders custom text when provided', () => {
    const output = renderToJson({ ...props, customText: 'Thousand gates' });
    expect(output).toContain('Thousand gates');
  });

  it('omits custom text when absent', () => {
    const output = renderToJson(props);
    expect(output).not.toContain('Thousand gates');
  });
});
