/**
 * MedallionStamp component tests.
...
import { MedallionStamp } from '@/components/stamps/MedallionStamp';
...
function renderToJson(props: Parameters<typeof MedallionStamp>[0]): string {
  const element = React.createElement(MedallionStamp, props);
...
describe('MedallionStamp', () => {
  ...
  it('renders custom text when provided', () => {
    const output = renderToJson({ ...props, customText: 'Bamboo grove' });
    expect(output).toContain('Bamboo grove');
  });

  it('omits custom text when absent', () => {
    const output = renderToJson(props);
    expect(output).not.toContain('Bamboo grove');
  });
});
