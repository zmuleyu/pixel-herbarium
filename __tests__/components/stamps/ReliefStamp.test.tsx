...
import { ReliefStamp } from '@/components/stamps/ReliefStamp';
...
function renderToJson(props: Parameters<typeof ReliefStamp>[0]): string {
  const element = React.createElement(ReliefStamp, props);
...
describe('ReliefStamp', () => {
  ...
  it('renders custom text when provided', () => {
    const output = renderToJson({ ...props, customText: 'Pure water temple' });
    expect(output).toContain('Pure water temple');
  });

  it('omits custom text when absent', () => {
    const output = renderToJson(props);
    expect(output).not.toContain('Pure water temple');
  });
});
