  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: plants } = await (supabase as any)
        .from('plants')
        .select('id, name_ja, hanakotoba')
        .in('id', plantIds);

      if (cancelled) return;

      const plantMap = new Map<number, { name_ja: string; hanakotoba: string }>(
        (plants ?? []).map((p: any) => [p.id, { name_ja: p.name_ja, hanakotoba: p.hanakotoba }]),
      );
